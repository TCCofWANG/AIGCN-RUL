...
class TCN_base(nn.Module):
    def __init__(self, num_nodes, in_dim, out_dim,d_model,d_ff,
        ...
        self.start_conv = nn.Conv2d(in_channels=in_dim,
                                        ...
                                        kernel_size=(1, 1))
        nn.init.xavier_uniform_(self.start_conv.weight)
        self.start_conv.bias = nn.Parameter(torch.zeros(self.start_conv.bias.shape, dtype=torch.double))
        ...
        self.residual_convs = nn.ModuleList([Conv2d(dilation_channels, residual_channels, (1, 1)) for _ in depth])
        self.skip_convs = nn.ModuleList([Conv2d(dilation_channels, skip_channels, (1, 1)) for _ in depth])
        for i in range(len(self.residual_convs)):
            nn.init.xavier_uniform_(self.residual_convs[i].weight)
            nn.init.xavier_uniform_(self.skip_convs[i].weight)
            self.residual_convs[i].bias = nn.Parameter(torch.zeros(residual_channels, dtype=torch.double))
            self.skip_convs[i].bias = nn.Parameter(torch.zeros(skip_channels, dtype=torch.double))
        ...
        for b in range(blocks):
            ...
            for i in range(layers):
                ...
                conv1 = Conv2d(residual_channels, dilation_channels, (1, kernel_size), dilation=D)
                nn.init.xavier_uniform_(conv1.weight)
                conv1.bias = nn.Parameter(torch.zeros(dilation_channels, dtype=torch.double))

                conv2 = Conv2d(residual_channels, dilation_channels, (1, kernel_size), dilation=D)
                nn.init.xavier_uniform_(conv2.weight)
                conv2.bias = nn.Parameter(torch.zeros(dilation_channels, dtype=torch.double))

                self.filter_convs.append(conv1)
                self.gate_convs.append(conv2)
                D *= 2
...
class GraphConvNet(nn.Module):
    def __init__(self, c_in, c_out, dropout, support_len=1, order=2, add_cross=False):
        ...
        self.final_conv.bias = nn.Parameter(torch.zeros(c_out, dtype=torch.double))
